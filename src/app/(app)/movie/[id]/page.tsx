...
import { extractYear, formatRuntime } from "@/lib/utils";
import { Rating } from "@/components/shared/Rating";
import Image from "next/image";
import { Credits, TMDB_BASE_IMG_URL} from "@/lib/tmdb";
import { useMemo, useState } from "react";
import TrailerPlaceholder from "@/components/ContentSection/TrailerPlaceholder";
...
const MoviePage = () => {
    const { id } = useParams();
    const { details, loading, error } = useMovieDetails(Number(id));
    ...
    const writers = useMemo(() => getWriters(details?.credits), [details?.credits]);
    ...
    return (
        <section className="flex flex-col gap-5 min-w-0">
            <BreadCrumb className="md:mt-5 font-medium text-base" items={[{ label: "Home", href: "/" }, { label: details?.title || "", href: `/movie/${id}` }]} />
            <Card className="p-6 rounded-3xl gap-5">
                <div>
                    <h1 className="text-[32px] font-semibold">{details?.title}</h1>
                    <div className="flex items-center gap-[3px]">
                        <span className="text-muted-foreground font-medium">
                            {extractYear(details?.release_date || "")} • {formatRuntime(details?.runtime || 0)} •
                        </span>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-[0.9fr_2.4fr]">
                  ...
                  <div className="relative aspect-video rounded-[20px] overflow-hidden">
                    ...
                    <TrailerPlaceholder details={details} onPlay={() => setIsTrailerOpen(true)} />
                )}
                </div>
                <div className="flex flex-col gap-4">
                        <div className="flex items-center gap-[9px]">
                            {details?.genres?.map((genre) => (
                                <Badge key={genre.id} variant="secondary" className="text-primary bg-white/10 py-2.5 px-3 rounded-[30px] text-xs">
                                    {genre.name}
                                </Badge>
                            ))}
                        </div>
            ...
            <section aria-label="Cast" className="flex flex-col gap-4  min-w-0 overflow-hidden flex-shrink-0">
                <div className="flex flex-row justify-between">
                    <h2 className="text-primary text-2xl font-semibold">Cast</h2>
                    <div className="flex flex-row gap-2">
                        <Button disabled={!api?.canScrollPrev} variant="secondary" className="cursor-pointer rounded-full bg-card aspect-square" onClick={() => api?.scrollPrev()} >
                            <ChevronLeft className="w-5 h-5" />
                        </Button>
                        <Button disabled={!api?.canScrollNext} variant="secondary" className="cursor-pointer rounded-full bg-card aspect-square" onClick={() => api?.scrollNext()}>
...
const Loading = () => {
  return (
    <section className="flex flex-col gap-5 min-w-0">
      ...
      {/* Cast */}
      <section aria-label="Cast" className="flex flex-col gap-4 min-w-0 overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="w-full aspect-[3/4] rounded-[20px] overflow-hidden bg-card">
