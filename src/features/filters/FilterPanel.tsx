...
const FilterPanel = ({ className }: { className?: string }) => {
  ...
  const href = {
    pathname: "/discover",
    query: {
      mediaType: Array.from(selectedMediaTypes)
        .map((mt) => mt.value)
        .join(","),
  ...
  const isDisabled = useMemo(() => {
    const ratingIsDefault =
      Number(rating_gte) === 1 && Number(rating_lte) === 10;
    return (
  ...
  return (
    <Card
      className={`shadow-none flex flex-col md:min-w-70 max-w-70 border-none gap-0 md:p-4 ${className}`}
    >
        ...
        <CardDescription className="flex justify-between items-center">
          ...
          >
            <Button
              disabled={isDisabled}
              type="button"
              className="cursor-pointer text-primary/50 hover:text-primary hover:bg-primary/10 bg-primary/10 rounded-[30px] text-sm font-medium"
              size="default"
            >
              {BUTTON_TEXT}
            </Button>
            ...
            <AccordionContent className="flex flex-row flex-wrap gap-2">
              {mediaTypes.map((type) => (
                <Toggle
                  ...
                  isActive={Array.from(selectedMediaTypes).some(
                    (mt) => mt.value === type.value,
                  )}
                >
                  {type.label}
                </Toggle>
            ...
            <AccordionContent className="flex flex-row flex-wrap gap-2">
              {availableGenres.map((genre, index) => (
                <Toggle
                  ...
                  isActive={Array.from(selectedGenres).some(
                    (g) => g.id === genre.id,
                  )}
                >
                  {genre.name}
          ...
          <AccordionItem value="item-3">
            ...
            <AccordionContent className="flex flex-row gap-2.5 items-center">
              <Input
                ...
                placeholder="1"
                className="rounded-[10px] ring-0 focus:ring-0 p-2.5"
              />
              <span>to</span>
              <Input
                type="number"
